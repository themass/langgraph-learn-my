...
class ChatState(TypedDict):
    ...
    next_node: str  # 新增：用于存储下一步要跳转的节点名称（"assistant"或"end"）


# =====================================================================
# 服务器连接测试
# =====================================================================
def ollama_connection(base_url="http://127.0.0.1:11434", timeout=5):
    """测试与Ollama服务器的连接
...
def human(state: ChatState) -> ChatState:
    ...
    # 返回更新后的状态
    return {"messages": messages}
def check_exit(state: ChatState) -> ChatState:
    # 复制当前状态（避免直接修改原状态）
    ...
    for msg in reversed(state["messages"]):
        if isinstance(msg, HumanMessage):
            ...
            if last_human_msg in ["exit", "quit", "退出"]:
                new_state["next_node"] = "end"  # 标记下一步退出
            else:
                new_state["next_node"] = "assistant"  # 标记下一步到AI回复
            break
    else:
        # 若没有人类消息，默认继续到AI回复
        new_state["next_node"] = "assistant"
    return new_state  # 返回更新后的状态字典
...
def build_graph():
    ...
    # 在图中添加判断节点和END连接
    workflow.add_node("check_exit", check_exit)

    # 条件分支：根据 check_exit 的返回值决定下一步
    # 路由函数直接返回 check_exit 的输出（"end" 或 "assistant"）
    # 条件分支：根据check_exit返回的状态中的next_node字段路由
    def route(state: ChatState) -> str:
        return state["next_node"]  # 返回下一步要跳转的节点名称

    workflow.add_conditional_edges(
        "check_exit",  # 源节点
        route  # 路由函数（基于状态中的next_node字段）
    )
    workflow.add_edge("assistant", "human")  # AI回复后回到用户输入
    workflow.add_edge("human", "check_exit")
    # workflow.add_edge("check_exit", "assistant", cond=lambda x: x == "assistant")
    # workflow.add_edge("check_exit", END, cond=lambda x: x == "end")
...
def main():
    ...
    # 测试与Ollama服务器的连接
    if not ollama_connection(OLLAMA_BASE_URL):
        print("\n❌ 无法连接到Ollama服务器，您可以:")
        print("1. 检查服务器地址是否正确")
        ...
        if use_local.lower() == 'y':
            ...
            if not ollama_connection(OLLAMA_BASE_URL):
                print("无法连接到服务器，退出程序。")
                return
            # if ollama_connection(new_base_url):
            #     # 重新创建LLM实例，使用本地服务器
            #     llm = OllamaLLM(
