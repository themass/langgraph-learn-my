...
async def run():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Call tool with name="Hello"
            print(f"🔧 Calling tool 'test_output_name' with name='Hello'...")
            result = await session.call_tool("test_output_name", arguments={"name": "Hello"})
