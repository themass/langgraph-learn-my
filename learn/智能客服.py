...
def user_input_node(state: CustomerServiceState) -> Dict[str, Any]:
    """处理用户输入的节点
    ...
    if user_messages:
        latest_message = user_messages[-1]["content"]
        # 添加用户消息到历史（如果还没有的话）
        if not any(msg["role"] == "user" and msg["content"] == latest_message for msg in messages):
            new_state["messages"] = messages + [{"role": "user", "content": latest_message}]
        else:
...
def run_example():
    ...
    for i, query in enumerate(sample_queries, 1):
        ...
        current_state["messages"] = [{"role": "user", "content": query}]
